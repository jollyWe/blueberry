<template>
  <div class="page-container">
    <!--工具栏-->
    <div class="toolbar" style="float:left;padding-top:10px;padding-left:10px;">
...
export default {
  ...
  methods: {
    ...
    handleDelete: function(data) {
      this.$api.modules.role.batchDelete(data.params).then(data.callback);
    },
    // 显示新增界面
    handleAdd: function() {
      ...
      this.$refs.dataForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            this.$api.modules.role.save(params).then(res => {
    ...
    handleRoleSelectChange(val) {
      ...
      this.$api.modules.role.findRoleMenus({ roleId: val.val.id }).then(res => {
    ...
    submitAuthForm() {
      ...
      this.$api.modules.role.saveRoleMenus(roleMenus).then(res => {
        if (res.code == 200) {
          this.$message({ message: "操作成功", type: "success" });
        } else {
